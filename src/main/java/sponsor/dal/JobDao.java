...
public class JobDao {
    ...
    public Job create(Job job) throws SQLException {
        String insertJob = "INSERT INTO Job(CASE_NUMBER, EMPLOYER_ID, JOB_TITLE, PW_SOC_CODE, PW_SOC_TITLE, PW_SKILL_LEVEL, PW_WAGE, WAGE_OFFER_FROM, WAGE_OFFER_TO, WAGE_OFFER_UNIT_OF_PAY, EMPLOYER_NAME) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
//        try (Connection connection = connectionManager.getConnection();
            ...
            statement.setString(11, job.getEmployerName());
...
//        try (Connection connection = connectionManager.getConnection();
            ...
            if (results.next()) {
                ...
                return new Job(caseNumber, employerId, jobTitle, pwSocCode, pwSocTitle, pwSkillLevel, pwWage, wageOfferFrom, wageOfferTo, wageOfferUnitOfPay, employerName);
            }
...
//        }
    ...
    public List<Job> getJobsByEmployerName(String employerName) throws SQLException {
        List<Job> jobs = new ArrayList<>();
        String selectByEmployerName = "SELECT * FROM Job WHERE EMPLOYER_NAME=?";
        
//        try (Connection connection = connectionManager.getConnection();
             PreparedStatement statement = connection.prepareStatement(selectByEmployerName);//) {
